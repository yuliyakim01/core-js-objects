...
function mergeObjects(objects) {
  return objects.reduce((acc, obj) => {
    Object.entries(obj).forEach(([key, value]) => {
      acc[key] = (acc[key] || 0) + value; // Sum values if key exists
    });
    return acc;
  }, {});
}
...
function sellTickets(queue) {
  const bills = { 25: 0, 50: 0, 100: 0 }; // Track available bills

  return queue.every((bill) => {
    bills[bill] = (bills[bill] || 0) + 1; // Increment the bill count

    if (bill === 50) {
      if (bills[25] > 0) {
        bills[25] -= 1; // Give one $25 as change
      } else {
    ...
    if (bill === 100) {
      if (bills[50] > 0 && bills[25] > 0) {
        bills[50] -= 1; // Give one $50
        bills[25] -= 1; // Give one $25
      } else if (bills[25] >= 3) {
        bills[25] -= 3; // Give three $25 bills
      } else {
        return false; // No change available
      }
    }
...
function fromJSON(proto, json) {
  const obj = JSON.parse(json); // Parse JSON into an object
  return Object.setPrototypeOf(obj, proto); // Assign the specified prototype
}
...
function sortCitiesArray(arr) {
  return arr.sort((a, b) => {
    if (a.country === b.country) {
      return a.city.localeCompare(b.city); // Sort by city if countries are the same
    }
...
class CssSelector {
  constructor() {
    this.selector = '';
    this.order = [];
    this.used = { element: false, id: false, pseudoElement: false };
  }
...
const cssSelectorBuilder = {
  ...
  combine: function combine(selector1, combinator, selector2) {
    return CssSelector.combine(selector1, combinator, selector2); // 🔥 Using the static method
  },
};
